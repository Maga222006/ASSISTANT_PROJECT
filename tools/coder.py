...
class Tool:
    def __init__(self):
        ...
        self.agent = initialize_agent(
            llm=self.llm,
            tools=[create_repo, commit_file_to_repo, read_file, list_files, list_repos, delete_file, list_branches,
                   create_branch],
        ...
        self.schema = {
            ...
            'function': {
                'name': 'coder',
                'description': 'A coding agent tool that handles coding tasks and saves all the code in GitHub. '
                               'Use whenever the user requests coding-related tasks.',
                'parameters': {
                    'type': 'object',
                    'properties': {
                        'query': {
                            'type': 'string',
                            'description': 'Detailed task for coding agent.'
                        }
                    },
                    'required': ['query'],
                },
    ...
    def run(self, query):
        """A GitHub coding agent that handles repositories, files, branches, and projects."""
        try:
